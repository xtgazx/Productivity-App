...
const initialAreaSeeds: AreaSeed[] = [
  {
    ...
    summary: "Personal routines and goals",
    preview: ["Morning routine", "Read more"],
    ideas: [
      { id: "personal-idea-1", title: "Start a simple journaling habit" },
      { id: "personal-idea-2", title: "Try a weekend reset routine" },
    ],
    projects: [
      {
        id: "personal-1",
  ...
  },
  {
    ...
    summary: "Fitness and recovery",
    preview: ["Walking plan", "Mobility"],
    ideas: [{ id: "health-idea-1", title: "Try a short daily walk after dinner" }],
    projects: [
      {
        id: "health-1",
  ...
  },
  {
    ...
    summary: "Projects and planning",
    preview: ["Weekly review", "Inbox cleanup"],
    ideas: [{ id: "work-idea-1", title: "Create a better meeting notes template" }],
    projects: [
      {
        id: "work-1",
    ...
    projects: [
      {
        id: "home-1",
        name: "Closet Cleanup",
        notes: "Reduce clutter and donate extra items.",
        dueDate: "",
        status: "Active",
        tasks: [
          {
            id: "home-1-t1",
            title: "Pull everything out",
            done: false,
            due: "",
            note: "",
            inWeek: true,
  ...
  },
  {
    ...
    summary: "Study and skill building",
    preview: ["Reading list", "Course notes"],
    ideas: [{ id: "learning-idea-1", title: "Make a one-page reading tracker" }],
    projects: [
      {
        id: "learning-1",
        name: "Reading Habit",
        notes: "Make reading feel easy to maintain.",
        dueDate: "",
        status: "Active",
        tasks: [
          {
  ...
  },
  {
    ...
    summary: "Life admin and paperwork",
    preview: ["Schedule appointment", "File documents"],
    ideas: [],
    projects: [
      {
        id: "admin-1",
        name: "Document Organization",
        notes: "Get important files into one place.",
        dueDate: "",
        status: "Active",
        tasks: [
          {
...
const initialBrainDumpItems: DumpItem[] = [
  { id: 1, text: "Look into a better weekly reset routine" },
  { id: 2, text: "Maybe create a packing checklist template" },
...
function BottomTab({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-1 flex-col items-center justify-center gap-1 rounded-2xl px-3 py-2 transition ${
        active ? "bg-indigo-100 text-indigo-700 shadow-sm" : "text-slate-500 hover:bg-slate-100"
      }`}
    >
  ...
  return (
    ...
    >
      <motion.button
        ...
        onClick={onOpen}
        className={`relative w-full rounded-2xl border border-slate-200 bg-gradient-to-b ${area.color} p-4 text-left shadow-lg shadow-slate-200/60 transition hover:-translate-y-0.5 hover:shadow-xl ${
      ...
      >
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-full bg-white/80 p-2 text-slate-500 shadow-sm hover:text-slate-700"
            ...
            <button
              ...
              onMouseDown={(e) => {
                e.stopPropagation();
                onStartDrag(e.clientX, e.clientY, tileRef.current);
              }}
              onTouchStart={(e) => {
                e.stopPropagation();
                const touch = e.touches[0];
                if (!touch) return;
                onStartDrag(touch.clientX, touch.clientY, tileRef.current);
              }}
              className="rounded-full bg-white/80 p-2 text-slate-400 shadow-sm"
        ...
        <div className="mt-1 text-xs text-slate-600">{area.projects.length} projects</div>
      </motion.button>
    </div>
  );
}
...
export default function App() {
  ...
  const [areas, setAreas] = useState<Area[]>(() => {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as {
        areas?: Array<Partial<Area> & { id: string; name: string }>;
      };
      return parsed.areas && Array.isArray(parsed.areas)
        ? hydrateAreas(parsed.areas)
        : initialAreas;
    } catch {
      return initialAreas;
    }
  ...
  const [brainDumpItems, setBrainDumpItems] = useState<DumpItem[]>(() => {
    if (typeof window === "undefined") return initialBrainDumpItems;
    try {
      const raw = window.localStorage.getItem(STORAGE_KEY);
      if (!raw) return initialBrainDumpItems;
      const parsed = JSON.parse(raw) as { brainDumpItems?: DumpItem[] };
      return parsed.brainDumpItems && Array.isArray(parsed.brainDumpItems)
        ? parsed.brainDumpItems.map((item) => ({
            id: Number(item.id),
            text: String(item.text),
          }))
        : initialBrainDumpItems;
    } catch {
      return initialBrainDumpItems;
    }
  });

  const [brainInput, setBrainInput] = useState("");
  const [selectedDumpId, setSelectedDumpId] = useState<number | null>(null);
  const [dumpTargetAreaId, setDumpTargetAreaId] = useState("");
  const [dumpTargetProjectId, setDumpTargetProjectId] = useState("");
  const [createProjectAreaId, setCreateProjectAreaId] = useState("");
  ...
  const [newAreaColor, setNewAreaColor] = useState("from-slate-100 to-zinc-50");
  const [editingAreaColorId, setEditingAreaColorId] = useState<string | null>(null);
  const [newInlineProjectAreaId, setNewInlineProjectAreaId] = useState<string | null>(null);
  const [inlineProjectName, setInlineProjectName] = useState("");
  const [inlineProjectNotes, setInlineProjectNotes] = useState("");
  ...
  const selectedDump = brainDumpItems.find((item) => item.id === selectedDumpId) || null;
  const dumpTargetArea = areas.find((area) => area.id === dumpTargetAreaId) || null;
  const draggedArea = dragState
    ? areas.find((area) => area.id === dragState.areaId) || null
    : null;
  ...
  useEffect(() => {
    ...
    const handleClickOutside = () => setAreaMenuOpen(null);
  ...
  useEffect(() => {
    ...
    try {
      const serializableAreas = areas.map(({ icon, ...area }) => area);
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ areas: serializableAreas, brainDumpItems })
      );
    } catch {}
  }, [areas, brainDumpItems]);
  ...
  useEffect(() => {
    ...
    const moveToPoint = (clientX: number, clientY: number) => {
      ...
      if (targetId && targetId !== dragState.areaId) {
        setAreas((current) => {
          const next = [...current];
          const fromIndex = next.findIndex((area) => area.id === dragState.areaId);
          const toIndex = next.findIndex((area) => area.id === targetId);
  ...
  const completedItems = useMemo(() => allTasks.filter((t) => t.done).slice(0, 3), [allTasks]);
                ...
                project.id !== projectId
                  ...
                  : {
                      ...
                      tasks: project.tasks.map((task) =>
                        task.id !== taskId ? task : updater(task)
                      ),
                    }
              ),
            }
      ...
      current.map((area) =>
        area.id !== areaId
          ...
          : {
              ...area,
              projects: area.projects.map((project) =>
                project.id !== projectId ? project : updater(project)
              ),
            }
      )
    );
  ...
  const addTaskToWeek = (areaId: string, projectId: string, taskId: string) =>
    updateTask(areaId, projectId, taskId, (task) => ({ ...task, inWeek: true }));

  const addTaskToToday = (areaId: string, projectId: string, taskId: string) =>
    updateTask(areaId, projectId, taskId, (task) => ({
      ...task,
      inToday: true,
      inWeek: true,
    }));

  const removeFromWeek = (areaId: string, projectId: string, taskId: string) =>
    updateTask(areaId, projectId, taskId, (task) => ({
      ...task,
      inWeek: false,
      inToday: false,
  ...
  const deleteBrainDumpItem = (id: number) => {
    setBrainDumpItems((current) => current.filter((item) => item.id !== id));
    if (selectedDumpId === id) setSelectedDumpId(null);
  };
  ...
  const saveAreaEdit = () => {
    ...
    setAreas((current) =>
      current.map((area) =>
        area.id !== editingAreaId ? area : { ...area, name: editingAreaName.trim() }
      )
    );
    setEditingAreaId(null);
    setEditingAreaName("");
  ...
  const saveAreaColor = (areaId: string, color: string) => {
    setAreas((current) =>
      current.map((area) => (area.id !== areaId ? area : { ...area, color }))
    );
    setEditingAreaColorId(null);
  };
              ...
              projects: [
                {
                  ...
                  notes: inlineProjectNotes.trim() || "New project.",
                  dueDate: "",
                  status: "Active",
                  tasks: [],
                },
  ...
  const startEditIdea = (ideaId: string, title: string) => {
    setEditingIdeaId(ideaId);
    setEditingIdeaTitle(title);
  };

  const saveIdeaEdit = (areaId: string) => {
    if (!editingIdeaId || !editingIdeaTitle.trim()) return;
    setAreas((current) =>
      current.map((area) =>
        area.id !== areaId
          ? area
          : {
              ...area,
              ideas: area.ideas.map((idea) =>
                idea.id !== editingIdeaId
                  ? idea
                  : { ...idea, title: editingIdeaTitle.trim() }
              ),
            }
      )
    );
  ...
  const deleteIdea = (areaId: string, ideaId: string) => {
    setAreas((current) =>
      current.map((area) =>
        area.id !== areaId
          ? area
  ...
  const convertIdeaToProject = (areaId: string, ideaId: string) => {
    const newProjectId = `${areaId}-project-${Date.now()}`;
  ...
  const addBrainDumpItem = () => {
    ...
    if (!text) return;
    setBrainDumpItems((current) => [{ id: Date.now(), text }, ...current]);
    setBrainInput("");
  };

  const resetBrainDumpRouting = () => {
    setSelectedDumpId(null);
    setDumpTargetAreaId("");
    setDumpTargetProjectId("");
    setCreateProjectAreaId("");
      ...
      current.map((area) =>
        area.id !== dumpTargetAreaId
          ...
          : {
              ...area,
              ideas: [
                { id: `${area.id}-idea-${Date.now()}`, title: selectedDump.text },
                ...area.ideas,
              ],
            }
      )
      ...
      current.map((area) =>
        area.id !== dumpTargetAreaId
          ...
          : {
              ...area,
              projects: area.projects.map((project) =>
                project.id !== dumpTargetProjectId
                  ? project
                  : {
                      ...project,
                      tasks: [
  ...
  const addArea = () => {
    ...
    setEditingAreaName("");
    setNewAreaName("");
    setNewAreaColor("from-slate-100 to-zinc-50");
    setIsNewAreaModalOpen(false);
  };

  const resetToTab = (nextTab: TabKey) => {
    setSelectedAreaId(null);
    setSelectedProjectId(null);
    setAreaMenuOpen(null);
    setTab(nextTab);
  ...
  const backOneLevel = () => {
    ...
    if (selectedArea) setSelectedAreaId(null);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto flex min-h-screen max-w-md flex-col bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.10),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(20,184,166,0.10),_transparent_28%),linear-gradient(180deg,_#f8fafc,_#eef2ff)]">
        <div className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 px-4 py-2 backdrop-blur-xl">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={saveChanges}
                className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50"
              >
                Save
              </button>
          ...
          {!selectedArea && !selectedProject && tab === "areas" && (
            <div className="mb-2 flex gap-2 overflow-x-auto pb-1">
              ...
              <StatPill label="This Week" value={weeklyItems.length} />
              <StatPill label="Today" value={todayItems.length} />
              <StatPill label="Brain Dump" value={brainDumpItems.length} />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-28 pt-4">
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-xl shadow-indigo-100/50">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <CardTitle className="text-base text-slate-900">Life Areas</CardTitle>
                    ...
                    >
                      <Plus className="mr-1 h-4 w-4" /> New Area
                    </Button>
                  </div>
                </CardHeader>
                    ...
                    return (
                      <div
                        ...
                        className={`relative space-y-2 overflow-visible ${
                          areaMenuOpen === area.id || dragState?.areaId === area.id
                            ? "z-50"
                            : "z-0"
                        }`}
                      >
                        {isEditing ? (
                          <Card className="rounded-2xl border-slate-200 bg-white/95 shadow-lg shadow-slate-200/60">
                            <CardContent className="space-y-3 p-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                <Button
                                  ...
                                  className="rounded-2xl bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50"
                                >
                                  Save
                                </Button>
                                <Button
                          ...
                          <>
                            <AreaTile
                              ...
                              isDragging={dragState?.areaId === area.id}
                              onOpen={() => {
                              ...
                              onToggleMenu={(e) => {
                                e.stopPropagation();
                                setAreaMenuOpen((current) =>
                                  current === area.id ? null : area.id
                                );
                              }}
                              onStartDrag={(clientX, clientY, element) =>
                                startDrag(area.id, clientX, clientY, element)
                              }
                              onEditName={() => {
                                startEditArea(area);
                                setAreaMenuOpen(null);
                              ...
                              onChangeColor={() => {
                                setEditingAreaColorId(area.id);
                                setAreaMenuOpen(null);
                              }}
                              onAddProject={() => {
                                setSelectedAreaId(area.id);
                                setNewInlineProjectAreaId(area.id);
                                setInlineProjectName("");
                                setInlineProjectNotes("");
                                setAreaMenuOpen(null);
                              }}
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                  {areaColorOptions.map((color) => (
                                    <button
                                      key={color}
                                      type="button"
                                      onClick={() => saveAreaColor(area.id, color)}
                                      className={`h-10 rounded-xl border border-slate-200 bg-gradient-to-br ${color}`}
                                      aria-label={color}
                                    />
                                  ))}
                                </div>
                ...
                <CardContent className="space-y-3">
                  {completedItems.map((item) => (
                    ...
                    >
                      ...
                      <span className="text-sm text-slate-500 line-through decoration-2 decoration-slate-400">
                        {item.title}
                      </span>
                    </div>
                  ))}
          ...
          {selectedArea && !selectedProject && (
            ...
            >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => startEditArea(selectedArea)}
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setEditingAreaColorId((current) =>
                            current === selectedArea.id ? null : selectedArea.id
                          )
                        }
                        className="rounded-full border-slate-200 bg-white/80 text-slate-700 hover:bg-white"
                      >
                        <Palette className="mr-1 h-4 w-4" /> Color
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => {
                          setNewInlineProjectAreaId((current) =>
                            current === selectedArea.id ? null : selectedArea.id
                          );
                          setInlineProjectName("");
                          setInlineProjectNotes("");
                        ...
                        className="rounded-full border-slate-200 bg-white/80 text-slate-700 hover:bg-white"
                      >
                        <FolderPlus className="mr-1 h-4 w-4" /> Add Project
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/95 shadow-lg shadow-slate-200/50">
                <CardHeader className="pb-3">
                  <button
                    type="button"
                  ...
                  >
                    <div>
                      <CardTitle className="text-base text-slate-900">Ideas</CardTitle>
                      <CardDescription className="text-slate-600">
                    ...
                    <ChevronRight
                      className={`h-4 w-4 text-slate-400 transition ${
                        ideasExpanded ? "rotate-90" : "rotate-0"
                      }`}
                    />
                  </button>
                </CardHeader>
                ...
                {ideasExpanded && (
                  <CardContent className="space-y-3">
                    <div className="flex gap-2">
                      ...
                      />
                      <Button
                        onClick={() => addIdeaToArea(selectedArea.id)}
                          ...
                          >
                            {editingIdeaId === idea.id ? (
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <Button
                                    onClick={() => saveIdeaEdit(selectedArea.id)}
                                    disabled={!editingIdeaTitle.trim()}
                                    className="rounded-2xl bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50"
                                  >
                                    Save
                                  </Button>
                              ...
                              <div className="flex items-center justify-between gap-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <Button
                                    ...
                                    variant="outline"
                                    onClick={() =>
                                      convertIdeaToProject(selectedArea.id, idea.id)
                                    }
                                  ...
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => startEditIdea(idea.id, idea.title)}
                                    className="rounded-full border-slate-200 bg-white text-slate-700 hover:bg-slate-100"
                                  >
                                    Edit
                                  </Button>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => deleteIdea(selectedArea.id, idea.id)}
                                    className="rounded-full border-red-200 bg-red-50 text-red-700 hover:bg-red-100"
                                  >
                                    Delete
                                  </Button>
              ...
              {editingAreaColorId === selectedArea.id && (
                <Card className="rounded-3xl border-slate-200 bg-white/95 shadow-lg shadow-slate-200/50">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base text-slate-900">Area Color</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-4 gap-2">
                      {areaColorOptions.map((color) => (
                        <button
                          key={color}
                          type="button"
                          onClick={() => saveAreaColor(selectedArea.id, color)}
                          className={`h-12 rounded-xl border border-slate-200 bg-gradient-to-br ${color}`}
                          aria-label={color}
                        />
                      ))}
                    </div>
              ...
              {newInlineProjectAreaId === selectedArea.id && (
                <Card className="rounded-3xl border-slate-200 bg-white/95 shadow-lg shadow-slate-200/50">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base text-slate-900">Add Project</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <Input
                    ...
                    />
                    <Textarea
                      value={inlineProjectNotes}
                      onChange={(e) => setInlineProjectNotes(e.target.value)}
                      placeholder="Project notes"
                      className="min-h-[90px] rounded-2xl border-slate-200 bg-slate-50 text-slate-900"
                    />
                    <div className="flex flex-wrap gap-2">
                      <Button
                        onClick={() => addInlineProjectToArea(selectedArea.id)}
              ...
              {selectedArea.projects.map((project) => (
                ...
                >
                  <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-lg shadow-slate-200/50 transition hover:shadow-xl">
                    <CardHeader className="pb-3">
            ...
            >
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-lg shadow-slate-200/50">
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-slate-600">
                    Context and controls for this project
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="text-sm text-slate-700">{selectedProject.notes}</div>
                  <div className="flex flex-wrap gap-2">
                    <MetaPill
                      icon={Calendar}
                      label={selectedProject.dueDate || "No due date"}
                    />
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        updateProject(selectedArea.id, selectedProject.id, (p) => ({
                          ...p,
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-lg shadow-slate-200/50">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <CardTitle className="text-base text-slate-900">Tasks</CardTitle>
                      <CardDescription className="text-slate-600">
                ...
                <CardContent className="space-y-3">
                  ...
                  {selectedProject.tasks.map((task) => (
                    <div
                      key={task.id}
                      className={`flex items-start gap-3 rounded-2xl border border-slate-200 px-3 py-3 ${
                        task.done ? "bg-slate-50 opacity-80" : "bg-white"
                      }`}
                    >
                      <button
                        onClick={() =>
                          toggleDone(selectedArea.id, selectedProject.id, task.id)
                        }
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          <button
                            onClick={() =>
                              addTaskToWeek(selectedArea.id, selectedProject.id, task.id)
                            }
                          >
                            <Badge
                ...
                <CardContent className="space-y-3">
                  {weeklyItems.map((item) => (
                    <div
                      key={item.id}
                      className={`flex items-start gap-3 rounded-2xl border border-slate-200 px-3 py-3 ${
                        item.done ? "bg-slate-50 opacity-80" : "bg-white"
                      }`}
                    >
                      <button
                        onClick={() => toggleDone(item.areaId, item.projectId, item.id)}
                        className="mt-0.5 shrink-0"
                      >
                        <CheckSquare
                          className={`h-5 w-5 ${
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          ...
                          {!item.inToday && (
                            <button
                              onClick={() =>
                                addTaskToToday(item.areaId, item.projectId, item.id)
                              }
                            >
          ...
          {!selectedArea && !selectedProject && tab === "today" && (
            ...
            >
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-xl shadow-emerald-100/40">
                ...
                <CardContent className="space-y-3">
                  {todayItems.map((item) => (
                    <div
                      key={item.id}
                      className={`flex items-start gap-3 rounded-2xl border border-slate-200 px-3 py-4 ${
                        item.done ? "bg-slate-50 opacity-80" : "bg-white"
                      }`}
                    >
                      <button
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          <button
                            onClick={() =>
                              removeFromToday(item.areaId, item.projectId, item.id)
                            }
                          >
                            <Badge className="rounded-full bg-slate-200 text-slate-700">
                              Remove from Today
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-lg shadow-slate-200/40">
                <CardHeader className="pb-3">
                  <CardTitle className="text-base text-slate-900">
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-xl shadow-fuchsia-100/40">
                ...
                <CardContent className="space-y-3">
                  <Textarea
                    ...
                    className="min-h-[110px] rounded-2xl border-slate-200 bg-slate-50 text-slate-900 placeholder:text-slate-500"
                  />
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <Plus className="mr-1 h-4 w-4" /> Add to Brain Dump
                    </Button>
                    <Button
                      variant="outline"
                ...
                <Card className="rounded-3xl border-slate-200 bg-white/95 shadow-lg shadow-slate-200/50">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <CardTitle className="text-base text-slate-900">
                          Sort Brain Dump Item
                        </CardTitle>
                        <CardDescription className="text-slate-600">
                          Choose where this item should go
                        </CardDescription>
                      </div>
                      <button
                        onClick={resetBrainDumpRouting}
                    ...
                    <div className="grid gap-4">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div className="grid gap-2">
                          <select
                            value={dumpTargetAreaId}
                            onChange={(e) => {
                              setDumpTargetAreaId(e.target.value);
                              setDumpTargetProjectId("");
                            }}
                          ...
                          <Button
                            onClick={sendDumpToIdea}
                            disabled={!dumpTargetAreaId}
                            className="rounded-2xl bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50"
                          >
                            Send to Area Idea
                          </Button>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div className="grid gap-2">
                          ...
                          {dumpTargetArea && (
                            <select
                              value={dumpTargetProjectId}
                              onChange={(e) => setDumpTargetProjectId(e.target.value)}
                              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
                            >
                              <option value="">Choose project</option>
                              {dumpTargetArea.projects.map((project) => (
                          ...
                          <Button
                            onClick={sendDumpToProject}
                            disabled={!dumpTargetAreaId || !dumpTargetProjectId}
                            className="rounded-2xl bg-indigo-600 text-white hover:bg-indigo-500 disabled:opacity-50"
                          >
                            Send to Project
                          </Button>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div className="grid gap-2">
                          <select
                            value={createProjectAreaId}
                            onChange={(e) => setCreateProjectAreaId(e.target.value)}
                            className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900"
                          >
                            <option value="">Choose area</option>
                            {areas.map((area) => (
                          ...
                          <Button
                            ...
                            className="rounded-2xl bg-fuchsia-600 text-white hover:bg-fuchsia-500 disabled:opacity-50"
                          >
                            Create Project From This
                          </Button>
                        </div>
              ...
              <Card className="rounded-3xl border-slate-200 bg-white/90 shadow-lg shadow-slate-200/40">
                ...
                <CardContent className="space-y-3">
                  {brainDumpItems.map((item) => (
                    ...
                    >
                      <div className="mb-2 text-sm text-slate-800">{item.text}</div>
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => setSelectedDumpId(item.id)}>
                          <Badge className="rounded-full bg-indigo-100 text-indigo-700">
                            Sort
                          </Badge>
                        </button>
        ...
        <div className="fixed inset-x-0 bottom-0 mx-auto max-w-md border-t border-slate-200 bg-white/90 px-4 pb-5 pt-3 backdrop-blur-xl">
          <div className="grid grid-cols-4 items-center gap-2 rounded-3xl border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-200/70">
            <BottomTab
              active={!selectedArea && !selectedProject && tab === "areas"}
              icon={Home}
              label="Areas"
              onClick={() => resetToTab("areas")}
            />
            <BottomTab
              active={!selectedArea && !selectedProject && tab === "week"}
              icon={CalendarDays}
              label="Week"
              onClick={() => resetToTab("week")}
            />
            <BottomTab
              active={!selectedArea && !selectedProject && tab === "today"}
              icon={CheckSquare}
              label="Today"
              onClick={() => resetToTab("today")}
            />
            <BottomTab
              active={!selectedArea && !selectedProject && tab === "brain"}
              icon={Brain}
              label="Brain"
              onClick={() => resetToTab("brain")}
            />
          </div>
        </div>
        ...
        {isNewAreaModalOpen && (
          <div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-900/35 px-4 pb-24 pt-8">
            <Card className="w-full max-w-md rounded-3xl border-slate-200 bg-white shadow-2xl shadow-slate-900/15">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <CardTitle className="text-base text-slate-900">New Area</CardTitle>
                    <CardDescription className="text-slate-600">
                      Create a new life area before adding projects.
                    </CardDescription>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    {areaColorOptions.map((color) => (
                      <button
                        ...
                        className={`h-10 rounded-xl border ${
                          newAreaColor === color ? "border-slate-900" : "border-slate-200"
        ...
        {confirmState && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/35 px-4 pb-24 pt-8">
            <Card className="w-full max-w-md rounded-3xl border-slate-200 bg-white shadow-2xl shadow-slate-900/15">
              <CardHeader className="pb-3">
                <CardTitle className="text-base text-slate-900">
                  Delete confirmation
                ...
                <CardDescription className="text-slate-600">
                  This will permanently remove “{confirmState.label}”.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
